...
write.table(data.frame(AGRETOPICITY=agretopicity,FOREIGNNESS=dis_score[match(aa,aa2),],HYDROPHOBICITY=hydrophobicity,data),paste(output,"/",sample,".dissimilarity.tsv",sep=""),sep='\t',row.names=F)