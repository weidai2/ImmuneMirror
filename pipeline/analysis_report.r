...
sample_name=tumor # "input_sample", ------------------------------------------- need to change
...
hla.II <- hla_II # --- need to change -------------------------------------------------------------------------------------- 2
#View(hla.II[1]) 
...
neo.I.raw.p <- ggplot(crc.info2,aes(
  ...
  geom_point(data = crc.info2, 
             aes(y=in.neo.I, # --- need to change ----------------------------------------------------------------------------- 3
                 x = 0),
...
Neo.I.raw.sample_val=in.neo.I # ------------------------------------------------need to change
...
Neo.I.filtered.sample_val=in.neo.I.filter # --------------------------------------------------------- need to change
...
Neo.II.raw.sample_val=in.neo.II # ---------------------------------------------------------------------- need to change
...
msi_sample_val=in.msi # --------------------------------------------------------------- need to change
...
lab_list<- c(1:219) # ---------------------------------------------------- need to change 
lab_list[1:219] <- ""
...
gene_sign.p1 <- as.ggplot(rasterGrob(readPNG("IPRES_Hmap.png",
 ...
 gene_sign.p <- gene_sign.p1 +
                     labs(title="I. IPRES gene signature")+
                 theme(plot.title=element_text(size =8 ,
                 face="bold"))
